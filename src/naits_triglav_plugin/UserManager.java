...
public class UserManager {
	...
	public ArrayList<String> loadPermissionPackages() {
		...
		permissionPackages.add(
				"ANIMAL_MOVEMENT_REGISTRATOR_PCK:HOLDING.FULL, HOLDING_RESPONSIBLE.FULL, ANIMAL.FULL, EAR_TAG_REPLC.READ, MOVEMENT_DOC.FULL, ANIMAL_MOVEMENT.FULL, VACCINATION_EVENT.FULL, VACCINATION_BOOK.FULL, VACCINATION_RESULTS.FULL, MOVEMENT_DOC_BLOCK.FULL, LAB_SAMPLE.READ, INVENTORY_ITEM.READ, ANIMAL_TYPE.READ, DISEASE.READ, PET.READ, FLOCK_MOVEMENT.READ, FLOCK.READ");
		permissionPackages.add(
				"FLOCK_MOVEMENT_REGISTRATOR:HOLDING.FULL, HOLDING_RESPONSIBLE.FULL,FLOCK_MOVEMENT.FULL, QUARANTINE.READ, VACCINATION_BOOK.FULL, VACCINATION_RESULTS.FULL, MOVEMENT_DOC_BLOCK.FULL,LAB_SAMPLE.READ, ANIMAL.READ, ANIMAL_MOVEMENT.READ, PET.READ");
		permissionPackages.add(
				"FULL_HOLDING_ADMINISTRATOR_PCK:HOLDING.FULL, HOLDING_RESPONSIBLE.FULL, ANIMAL.FULL, FLOCK.FULL, EAR_TAG_REPLC.FULL, MOVEMENT_DOC.FULL, FLOCK_MOVEMENT.FULL, ANIMAL_MOVEMENT.FULL, QUARANTINE.READ, VACCINATION_EVENT.FULL, VACCINATION_BOOK.FULL, VACCINATION_RESULTS.FULL, MOVEMENT_DOC_BLOCK.FULL, SPOT_CHECK.FULL, LAB_SAMPLE.READ, INVENTORY_ITEM.FULL, ANIMAL_TYPE.READ, DISEASE.READ, PET.FULL");
		permissionPackages.add(
		...
		permissionPackages.add(
				"FVIRO_PCK:HOLDING.READ, ANIMAL.READ, HOLDING_RESPONSIBLE.READ, LAB_SAMPLE.FULL, LAB_TEST_RESULT.READ, LABORATORY.READ, VACCINATION_BOOK.FULL, VACCINATION_RESULTS.FULL, VACCINATION_EVENT.FULL, ANIMAL_TYPE.READ, DISEASE.READ, ANIMAL_MOVEMENT.READ, PET.READ, FLOCK_MOVEMENT.READ, FLOCK.READ");
		permissionPackages.add(
				"CVIRO_PCK:HOLDING.READ, ANIMAL.READ, HOLDING_RESPONSIBLE.READ, LAB_SAMPLE.FULL, LAB_TEST_RESULT.FULL, LABORATORY.READ, VACCINATION_BOOK.FULL, VACCINATION_EVENT.FULL, VACCINATION_RESULTS.FULL, ANIMAL_TYPE.READ, DISEASE.READ, ANIMAL_MOVEMENT.READ, PET.READ, FLOCK_MOVEMENT.READ, FLOCK.READ");
		permissionPackages.add(
				"LABORANT_PCK:LABORATORY.FULL, LAB_SAMPLE.FULL, LAB_TEST_RESULT.FULL,LAB_TEST_TYPE.FULL, VACCINATION_RESULTS.FULL");
		...
		permissionPackages.add(
				"MOVEMENT_DOC_SEARCH_PCK:custom.movement_doc_search, HOLDING.READ, HOLDING_RESPONSIBLE.READ, ANIMAL.READ, EAR_TAG_REPLC.READ, MOVEMENT_DOC.READ, ANIMAL_MOVEMENT.FULL, MOVEMENT_DOC_BLOCK.FULL, INVENTORY_ITEM.READ, ANIMAL_TYPE.READ, PET.READ, FLOCK_MOVEMENT.READ, FLOCK.READ");
				...
				for (int i = 0; i < jsonData.size(); i++) {
					...
					try {
						...
						try {
							...
							if (convertedAllDefaultGroups.size() > 0) {
							...
							if (convertedAllAdditionalGroups.size() > 0) {
			...
			for (int i = 0; i < jsonData.size(); i++) {
				...
				try {
					...
					if (actionName.equals(Tc.ASSIGN)) {
						ArrayList<String> currentPermissions = getCurrentPermissionsPerUserOrGroup(dboObjectToHandle,
								svr);
					...
					processPermissionsToUserOrGroup(actionName, dboObjectToHandle, null, filteredPackagePermissions,
							false, svr, svw, svs);
	...
	public void attachPermissionsToUserGroup(String permissionPackageName, DbDataObject groupObj,
		...
		try {
			...
			if (permissionPackageName.equals("ALL.READ") || permissionPackageName.equals("ALL.FULL")) {
				...
				if (coreTablePermissions.size() > 0)
			...
			if (customTablePermissions.size() > 0)
				setSidPermission(groupObj.getVal(Tc.GROUP_NAME).toString(), svCONST.OBJECT_TYPE_GROUP,
	...
	public void processPermissionsToUserOrGroup(String processType, DbDataObject userOrGroupObj,
		...
		if (coreTablePermissions != null && coreTablePermissions.size() > 0) {
		...
		if (customTablePermissions != null && customTablePermissions.size() > 0) {
			setSidPermission(userOrGroupObj.getVal(userOrGroupName).toString(), userOrGroupObj.getObject_type(),
		...
		if (objToGetPermissionsFrom.getObject_type().equals(svCONST.OBJECT_TYPE_GROUP)) {
			...
			if (groupObj.getVal(Tc.GROUP_NAME) != null) {
				...
				if (setSidPermission(groupObj.getVal(Tc.GROUP_NAME).toString(), svCONST.OBJECT_TYPE_GROUP,
						currentPermissions, Tc.REVOKE, svr, svw, svs))
	...
	public ArrayList<String> getCurrentGroupPermissions(String groupName, SvReader svr) {
		...
		try {
			...
			if (sid != null) {
				...
				if (currentGroupPrmissions != null && currentGroupPrmissions.size() > 0) {
	...
	public ArrayList<String> getCurrentUserPermissions(String userName, SvReader svr) {
		...
		try {
			...
			if (sid != null) {
				...
				if (currentUserPrmissions != null && currentUserPrmissions.size() > 0) {
	...
	public boolean setSidPermission(String sidName, Long sidType, ArrayList<String> permissionKeys, String operation,
			SvReader svr, SvWriter svw, SvSecurity svs) {
		DbDataObject sid = null;
		try {
	...
	public DbDataObject createDefaultTestUser(String userName, String firstName, String lastName, String email,
		...
		if (dboUser == null) {
			...
			dboUser.setVal(Tc.PASSWORD_HASH, SvUtil.getMD5(password));
		} else {
	...
	public Boolean checkIfUserHasAdmGroup(DbDataObject dboUser, SvReader svr) throws SvException {
		...
		if (result == false && convertedAllAdditionalGroups.size() > 0) {
	...
	public ArrayList<String> getCustomPermissionForUserOrGroup(DbDataObject objToGetPermissionsFrom, SvReader svr)
			throws SvException {
		ArrayList<String> result = new ArrayList<String>();
		ArrayList<String> temp_result = getCurrentPermissionsPerUserOrGroup(objToGetPermissionsFrom, svr);
		for (int i = 0; i < temp_result.size(); i++) {
			if (!temp_result.get(i).toUpperCase().contains(Tc.SVAROG)
					&& !temp_result.get(i).toUpperCase().contains(Tc.SYSTEM)) {
	...
	public String detachUsersToOrgUnit(Long orgUnitId, JsonArray jsonData, String sessionId) throws Exception {
		...
		try {
				...
				try {
					...
					try {
						if (isGroupType) {
							...
							userGroupInformation.add(dboObjectToHandle.getVal(Tc.GROUP_NAME) != null
								...
								for (DbDataObject dboUser : arrayAttachedUsers.getItems()) {
									...
									if (!subOrgUnits.getItems().isEmpty()) {
										for (DbDataObject dboSubOrgUnit : subOrgUnits.getItems()) {
											...
											if (dbLinkObj != null) {
												linkDelete(dboUser, dboSubOrgUnit, Tc.POA, svr, svw, svl);
								...
								if (!subOrgUnits.getItems().isEmpty()) {
									for (DbDataObject dboSubOrgUnit : subOrgUnits.getItems()) {
										DbDataObject dbLinkObj = rdr.getLinkObject(dboObjectToHandle.getObject_id(),
												dboSubOrgUnit.getObject_id(), dbLinkType.getObject_id(), svr);
										if (dbLinkObj != null) {
											linkDelete(dboObjectToHandle, dboSubOrgUnit, Tc.POA, svr, svw, svl);
										} else {
											if (dboSubOrgUnit.getObject_id().equals(dboOrgUnit.getObject_id())) {
			...
			svw.dbCommit();
			if (!isHeadquarter) {
				if (usersNotLinkedToOrgUnit != null && usersNotLinkedToOrgUnit.size() > 0) {
	...
	public String assignUserToObjectWithPoa(Long objectId, JsonArray jsonData, String sessionId) throws Exception {
		...
		try {
			...
			if (usersWithSameLaboratory != null && usersWithSameLaboratory.size() > 0) {
				result = I18n.getText(svr.getUserLocaleId(svr.getInstanceUser()),
	...
	public String unassignUserFromObjectWithPoa(Long objectId, JsonArray jsonData, String sessionId) throws Exception {
		...
		try {
				...
				for (int i = 0; i < jsonData.size(); i++) {
					...
					try {
						...
						try {
							...
							if (dbLinkObj != null) {
								linkDelete(dboObjectToHandle, dboToLink, customPoa + Tc.POA, svr, svw, svl);
							} else {
			...
			if (usersNotLinkedToLaboratory != null && usersNotLinkedToLaboratory.size() > 0) {
				result = I18n.getText(svr.getUserLocaleId(svr.getInstanceUser()),
			...
			if (dboUser != null)
				...
				for (int i = 0; i < jsonData.size(); i++) {
					...
					try {
						...
						try {
							linkDelete(dboUser, dboObjectToHandle, Tc.POA, svr, svw, svl);
						} catch (SvException e) {
							log4j.info(e.getFormattedMessage());
						}
					} finally {
	...
	public String createPOABetweenUserAndOrgUnit(DbDataObject dboUser, DbDataObject dboOrgUnit, SvReader svr) {
		...
		try {
			...
			if (linkObj != null) {
				...
				svl.linkObjects(dboUser.getObject_id(), dboOrgUnit.getObject_id(), dblink.getObject_id(), "", true,
						false);
				svl.dbCommit();
