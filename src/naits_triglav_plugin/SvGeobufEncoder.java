...
/**
 *	Svarog extension of GeobufEncoder
 * 	<br>	
 *
 */
	...
	private void setProperties(Geobuf.Data.Feature.Builder featBld, GeobufFeature feature, List<String> keys) {
		...
		for (Map.Entry<String, Object> e : feature.properties.entrySet()) {
            ...
            if (featVal instanceof Geometry) {
                ...
                log4j.debug("Unable to save object of type " + featVal.getClass().getTypeName() +" to geobuf, falling back on toString. Deserialization will not work as expected.", featVal.getClass());
                val.setStringValue(featVal.toString());
            }
 ...
 * wip, look elsewhere
 * <br>
 * how to map custom geometry (custom field name/manual construction/null) to dbDataObject values map ? 
 * 
 * @param dbArr
 * @throws IOException
 */
